...
<body>
    ...
    def test_filename_generation_company_name_normalization(
        ...
        for company_name, expected_base in test_cases:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                assert os.path.exists(
                    expected_html
                ), f"Expected HTML file {expected_html} for company '{company_name}'"
                assert os.path.exists(
                    expected_pdf
                ), f"Expected PDF file {expected_pdf} for company '{company_name}'"

    def test_generate_files_different_output_directory(
        self, sample_invoice_data, mock_template_html
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_pdf = os.path.join(
                output_dir, "acme-corporation-invoice-03.15.2025.pdf"
            )

            assert os.path.exists(expected_html)
            assert os.path.exists(expected_pdf)

    def test_generate_files_with_multiple_items(
        self, sample_invoice_data, mock_template_html
    ):
    ...
    def test_generate_files_with_zero_total(
        self, sample_invoice_data, mock_template_html
    ):
    ...
    def test_generate_files_with_decimal_amounts(
        self, sample_invoice_data, mock_template_html
    ):
    ...
    def test_generate_files_invalid_output_directory(
        self, sample_invoice_data, mock_template_html
    ):
    ...
    def test_generate_files_special_characters_in_client_name(
        self, sample_invoice_data, mock_template_html
    ):
    ...
    def test_generate_files_empty_items_list(
        self, sample_invoice_data, mock_template_html
    ):
    ...
    def test_generate_files_date_format_variations(
        self, sample_invoice_data, mock_template_html
    ):
        ...
        for input_date, expected_date in date_formats:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                expected_html = os.path.join(
                    temp_dir, f"acme-corporation-invoice-{expected_date}.html"
                )
                assert os.path.exists(
                    expected_html
                ), f"Expected file for date {input_date}"
